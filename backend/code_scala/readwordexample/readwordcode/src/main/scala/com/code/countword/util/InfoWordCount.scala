...
class InfoWordCount(lengthWord: Int, qtaWord: Int, print: String=>Unit) extends AbstractExecutionContext {
  ...
  def merge(map: Map[String, Int], file: File, task:Int, numTask: Int):Future[Unit]=Future{
  ...
  def sendWordCount():Unit=
    Future{wordFrequency.entrySet().asScala.map(value=>value.getKey->value.getValue)
      .toSeq.sortWith(_._2>_._2)}.map(result=>
       result.take(qtaWord).map(value=>InfoWord(value._1,value._2)).mkString("\n")).map(string=>print(string))
